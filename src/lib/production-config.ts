...
export function validateProductionConfig(env: NodeJS.ProcessEnv = process.env): ConfigIssue[] {
  ...
  if (production && !["full", "demo"].includes(mode ?? "")) issues.push({ severity: "error", key: "DEPLOYMENT_MODE", message: "Defina explicitamente DEPLOYMENT_MODE=full ou demo." });
  ...
  if (production && mode === "full" && env.STORAGE_PROVIDER !== "s3") issues.push({ severity: "error", key: "STORAGE_PROVIDER", message: "Produção full exige storage privado persistente S3-compatible." });
  if (env.STORAGE_PROVIDER === "s3" && mode === "full") for (const key of ["STORAGE_BUCKET", "STORAGE_REGION", "STORAGE_ACCESS_KEY_ID", "STORAGE_SECRET_ACCESS_KEY"]) requireKey(key);
  ...
  if (mode === "full" && env.JOB_EXECUTOR === "disabled") issues.push({ severity: production ? "error" : "warning", key: "JOB_EXECUTOR", message: "JOB_EXECUTOR=disabled é permitido somente no modo demo." });
