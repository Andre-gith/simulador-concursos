...
export class InlineJobExecutor implements JobExecutor {
  ...
  constructor(environment = process.env) {
    if (environment.NODE_ENV === "production" && environment.DEPLOYMENT_MODE !== "lite") throw new Error("Executor inline é proibido em produção fora do modo Lite.");
  }
...
export function jobExecutor(env = process.env): JobExecutor {
  ...
  if (!["full", "demo", "lite"].includes(deploymentMode ?? "")) throw new Error("DEPLOYMENT_MODE inválido.");
  if (deploymentMode === "demo" && env.JOB_EXECUTOR !== "disabled") throw new Error("JOB_EXECUTOR=disabled é obrigatório no modo demo.");
  if (deploymentMode === "lite" && env.JOB_EXECUTOR !== "disabled") throw new Error("JOB_EXECUTOR=disabled é obrigatório no modo Lite.");
  ...
  singleton ??= deploymentMode === "lite" && env.JOB_EXECUTOR === "disabled"
    ? new InlineJobExecutor(env)
    : env.JOB_EXECUTOR === "disabled"
