...
export async function GET() {
  try {
    ...
    if (!isPrivateStorageConfigured()) return Response.json({ status: "not_ready", checks: { configuration: true, database: true, storage: false } }, { status: 503, headers: { "cache-control": "no-store" } });
    privateStorage();
