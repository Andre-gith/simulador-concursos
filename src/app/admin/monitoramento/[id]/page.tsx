...
export default async function MonitorPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{error?:string;notice?:string}>}) {
  ...
  {demo&&<p className="mt-4 rounded-xl border border-amber-300 bg-amber-50 p-4 font-bold text-amber-900">{DEMO_AUTOMATION_MESSAGE}</p>}
  ...
  <div className="mt-5 flex gap-3"><form action={runMonitorAction}><input type="hidden" name="monitorId" value={monitor.id}/><button disabled={demo} className="rounded-xl bg-emerald-900 px-5 py-3 font-black text-white disabled:cursor-not-allowed disabled:opacity-40">Executar verificação agora</button></form><form action={toggleMonitorAction}><input type="hidden" name="monitorId" value={monitor.id}/><input type="hidden" name="enabled" value={monitor.enabled?"false":"true"}/><button disabled={demo} className="rounded-xl border px-5 py-3 font-black disabled:cursor-not-allowed disabled:opacity-40">{monitor.enabled?"Desativar":"Ativar"}</button></form></div>
