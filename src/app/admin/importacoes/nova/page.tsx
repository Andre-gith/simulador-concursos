...
export default async function NewOfficialImportPage({ searchParams }: { searchParams: Promise<{ error?: string; editorialCatalogEntryId?: string }> }) {
  ...
  return <AdminShell><main className="mx-auto max-w-5xl px-4 py-10">
    ...
    <div className="mt-6 rounded-3xl border border-amber-200 bg-[#fffdf5] p-6 shadow-sm">
      ...
      <OfficialImportForm disabled={demo} defaults={entry ? { editorialCatalogEntryId: entry.id, institution: entry.orgao, board: entry.banca?.name, position: entry.cargo ?? entry.title, specialty: entry.especialidade ?? undefined, year: entry.ano ?? undefined, edition: entry.edicao ?? undefined } : {}} />
    </div>
  </main></AdminShell>;
}
